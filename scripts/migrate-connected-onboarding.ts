...
try {
  ...
  if (
    ...
    identity.fingerprint !== TASK_ANNOTATION_SCHEMA_FINGERPRINT ||
    databaseSchemaFingerprint(database) !== TASK_ANNOTATION_SCHEMA_FINGERPRINT
  ) {
    throw new Error(
      "Database is not the exact supported Task Annotation schema; no changes were made",
    )
  }
  ...
  database
    .transaction(() => {
      for (const statement of connectedOnboardingSchemaStatements()) database.query(statement).run()
      database
