...
import { verifyDeployedCollaborationProofReceipt } from "./deployed-collaboration-proof-receipt"
...
if (pathArguments.length !== 1) {
  throw new TypeError(
    "Usage: bun scripts/verify-deployed-collaboration-proof.ts <receipt> [--require-clean] [--commit=<sha>]",
  )
}
...
const receipt = verifyDeployedCollaborationProofReceipt(
  await Bun.file(resolve(pathArguments[0] as string)).json(),
)
if (requireClean && receipt.revision.dirty) {
  throw new TypeError("Deployed collaboration proof was produced from a dirty worktree")
}
if (expectedCommit !== undefined && receipt.revision.commit !== expectedCommit) {
  throw new TypeError("Deployed collaboration proof belongs to a different Git revision")
}
