...
try {
  ...
  if (
    ...
    identity.fingerprint !== TASK_RELAY_SCHEMA_FINGERPRINT ||
    databaseSchemaFingerprint(database) !== TASK_RELAY_SCHEMA_FINGERPRINT
  ) {
    throw new Error("Database is not the exact supported Task Relay schema; no changes were made")
  }
  ...
  const summary = {
    ...
    relays: count(database, "task_relays"),
  }
  if (!write) {
    console.log(
      JSON.stringify(
  ...
  database
    .transaction(() => {
      for (const statement of taskAnnotationSchemaStatements()) database.query(statement).run()
      database
