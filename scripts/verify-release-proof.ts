...
import { verifyReleaseProofReceipt } from "./release-proof-receipt"
...
if (pathArguments.length !== 1) {
  throw new TypeError(
    "Usage: bun scripts/verify-release-proof.ts <receipt> [--require-clean] [--commit=<sha>]",
  )
}
...
const receipt = verifyReleaseProofReceipt(
  await Bun.file(resolve(pathArguments[0] as string)).json(),
)
if (requireClean && receipt.revision.dirty) {
  throw new TypeError("Release proof receipt was produced from a dirty worktree")
}
if (expectedCommit !== undefined && receipt.revision.commit !== expectedCommit) {
  throw new TypeError("Release proof receipt belongs to a different Git revision")
}
