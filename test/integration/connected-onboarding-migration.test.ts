...
const TASK_ANNOTATION_SCHEMA_FINGERPRINT =
  "7769934fcf2e919d0d9ac6093f084ba225c2daddddce1f44243105a826d3ea01"
...
test("the exact Task Annotation migration adds empty connected-onboarding state", async () => {
  temporary = await mkdtemp(join(tmpdir(), "meanwhile-onboarding-migration-"))
  const path = join(temporary, "meanwhile.sqlite")
  const legacy = new Database(path, { strict: true })
  for (const statement of taskAnnotationSchemaStatements()) legacy.query(statement).run()
  expect(databaseSchemaFingerprint(legacy)).toBe(TASK_ANNOTATION_SCHEMA_FINGERPRINT)
  legacy
    .query("INSERT INTO schema_identity VALUES (1,?,?,?)")
    .run(CURRENT_SCHEMA.name, TASK_ANNOTATION_SCHEMA_FINGERPRINT, "2026-07-24T00:00:00.000Z")
  legacy.close()

  const migration = Bun.spawn(
    [
      Bun.which("bun") ?? "bun",
      "scripts/migrate-connected-onboarding.ts",
      `--database=${path}`,
      "--write",
    ],
  ...
  try {
    ...
    for (const table of [
      ...
      expect(
        migrated.query<{ count: number }, []>(`SELECT COUNT(*) AS count FROM ${table}`).get()
          ?.count,
      ).toBe(0)
    }
  } finally {
...
function taskAnnotationSchemaStatements(): readonly string[] {
  const onboardingObjects = new Set([
    "external_identities",
  ...
  return splitSchemaSql(SCHEMA_SQL).filter((statement) => {
    ...
    return match === null || !onboardingObjects.has(match[1] as string)
  })
}
