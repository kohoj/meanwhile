...
test("execution context freezes artifact evidence and renders it as untrusted prior observation", async () => {
  ...
  const context = new ExecutionContext(reader)
  ...
  const prompt = await context.renderPrompt(OWNER_ID, accepted, "Fix the current checkout")
...
test("execution context rejects non-text and oversized artifact entries", async () => {
  ...
  await expect(
    new ExecutionContext(binary).resolve(OWNER_ID, [{ artifactId: ARTIFACT_ID }]),
  ...
  await expect(
    new ExecutionContext(oversized).resolve(OWNER_ID, [{ artifactId: ARTIFACT_ID }]),
  ).rejects.toMatchObject({ code: "INVALID_REQUEST" })
})
...
test("execution context fails closed if accepted source evidence changes", async () => {
  ...
  await expect(context.renderPrompt(OWNER_ID, accepted, "continue")).rejects.toMatchObject({
...
test("execution context cannot be terminated by artifact-controlled delimiter text", async () => {
  ...
  const prompt = await context.renderPrompt(OWNER_ID, accepted, "Continue safely")
